...
if not is_module_connected() then
    m = Map("modem-AK68", translate("模组已断开或未接入！请接入后重试。"))
    return m
end

m = Map("modem-AK68", translate("C5800/AK68移动网络设置"))
section = m:section(TypedSection, "ndis", translate("C5800/AK68模组设置-移远RM520N"))
...
simsel= section:taboption("general", ListValue, "simsel", translate("卡槽选择-部分仅限内置X62"))
simsel:value("0", translate("外置SIM卡1"))
simsel:value("1", translate("内置SIM1"))
simsel:value("2", translate("内置SIM2-仅限内置X62"))
simsel:value("3", translate("外置SIM卡2-仅限内置X62"))
simsel:value("4", translate("外置SIM卡3-仅限内置X62"))
simsel:value("5", translate("外置SIM卡4-仅限内置X62"))
