    ...
    if [ ${Enable_PING} == 1 ];then
     ...
     atsd_tools_cli -i cpe -c 'AT+QSIMDET=1,0'
     #开启热插拔检测低电平
     atsd_tools_cli -i cpe -c 'AT+QSIMSTAT=1'
     #开启自动上报卡检测
        case "$Sim_Sel" in
            0)
                printMsg "外置SIM卡"
                ...
                echo "等待GPIO生效,外置SIM卡1切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            1)
                printMsg "内置SIM1"
                echo 1 > /sys/class/gpio/cpe-sel3/value
                ...
                echo "等待GPIO生效,内置SIM卡1切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            2)
                printMsg "内置SIM2"
                ...
                echo "等待GPIO生效,内置SIM卡2切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            3)
                printMsg "外置SIM2"
                ...
                echo "等待GPIO生效,外置SIM卡2切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            4)
                printMsg "外置SIM3"
                ...
                echo "等待GPIO生效,外置SIM卡3切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            5)
                printMsg "外置SIM4"
                ...
                echo "等待GPIO生效,外置SIM卡4切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            *)
                printMsg "错误状态"
                ...
                echo "SIM卡槽选择数据未正确设定,默认设定外置卡1" >> /tmp/moduleInit-AK68
                echo "SIM卡槽选择数据未正确设定,默认设定外置卡1,等待GPIO生效,外置SIM卡1切换中...">/tmp/simcardstat-AK68
                atsd_tools_cli -i cpe -c 'at+cfun=1,1'
            ;;
            esac
    ...
    chkSimExt() {
    	while true; do
	    response=$(tsd_tools_cli -i cpe -c 'ATI')
