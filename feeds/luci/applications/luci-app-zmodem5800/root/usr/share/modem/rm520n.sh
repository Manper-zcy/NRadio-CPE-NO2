    ...
    VAR=""
    if [ ! -f /etc/config/simst1.conf ]; then
    	touch /etc/config/simst1.conf
    fi

    current_value=$(cat /etc/config/simst1.conf)
    cat /etc/config/simst1.conf
    if [ -z "$current_value" ]; then
    	echo "Error: /etc/config/simst1.conf 值不存在."
	echo "0" > /etc/config/simst1.conf
    fi

    if [ "$current_value" -eq 1 ]; then
    	sendat 2 'AT+QSIMDET=1,0'
    	#开启热插拔检测低电平
    	echo "0" > /etc/config/simst1.conf
    elif [ "$current_value" -eq 0 ]; then
    	sendat 2 'AT+QSIMDET=1,1'
    	#开启热插拔检测低高电平
    	echo "1" > /etc/config/simst1.conf
    else
    	echo "Error: 读取错误 /etc/config/simst1.conf"
    fi

    #sendat 2 'AT+QSIMDET=1,1'
    #开启热插拔检测低电平/高电平
    sendat 2 'AT+QSIMSTAT=1,0'  
    #开启自动上报卡检测
        case "$Sim_Sel" in
            0)
                printMsg "外置SIM卡1"
                ...
                echo 0 > /tmp/sim_sel
                echo "等待GPIO生效,外置SIM卡1切换中...">/tmp/simcardstat
	            echo 0 > /sys/class/gpio/cpe-pwr/value
	            sleep 3
	            echo 1 > /sys/class/gpio/cpe-pwr/value
                #sleep 10
            ...
            1)
                ...
                echo 1 > /tmp/sim_sel
                echo "等待GPIO生效,内置SIM卡1切换中...">/tmp/simcardstat
                echo 0 > /sys/class/gpio/cpe-pwr/value
                sleep 3
                echo 1 > /sys/class/gpio/cpe-pwr/value
            ;;
            2)
                ...
                echo 2 > /tmp/sim_sel
                echo "等待GPIO生效,内置SIM卡2切换中...">/tmp/simcardstat
                echo 0 > /sys/class/gpio/cpe-pwr/value
                sleep 3
                echo 1 > /sys/class/gpio/cpe-pwr/value
            ;;
            3)
                ...
                echo 3 > /tmp/sim_sel
                echo "等待GPIO生效,外置SIM卡2切换中...">/tmp/simcardstat
                echo 0 > /sys/class/gpio/cpe-pwr/value
                sleep 3
                echo 1 > /sys/class/gpio/cpe-pwr/value
            ;;
            4)
                ...
                echo 4 > /tmp/sim_sel
                echo "等待GPIO生效,外置SIM卡3切换中...">/tmp/simcardstat
                echo 0 > /sys/class/gpio/cpe-pwr/value
                sleep 3
                echo 1 > /sys/class/gpio/cpe-pwr/value
            ;;
            5)
                ...
                echo 5 > /tmp/sim_sel
                echo "等待GPIO生效,外置SIM卡4切换中...">/tmp/simcardstat
                echo 0 > /sys/class/gpio/cpe-pwr/value
                sleep 3
                echo 1 > /sys/class/gpio/cpe-pwr/value
            ;;
            *)
                ...
                sendat 2 'AT+QMBNCFG="autosel",1' 
                echo 6 > /tmp/Sim_Sel
                echo "SIM卡槽选择数据未正确设定,默认设定外置卡1" >> /tmp/moduleInit
                echo "SIM卡槽选择数据未正确设定,默认设定外置卡1,等待GPIO生效,外置SIM卡1切换中...">/tmp/simcardstat
                echo 0 > /sys/class/gpio/cpe-pwr/value
                sleep 3
                echo 1 > /sys/class/gpio/cpe-pwr/value
            ;;
    ...
    chkSimExt() {
	while true; do
	    response=$(sendat 2 'ATI')
	    if [[ "$response" == *"RM520N"* ]]; then
		echo "Detected 'RM520N' in the response."
		break 
	    else
		echo "未检测到模组上线,继续检测模组是否上线..."
		sleep 5
	    fi
	done
	echo "检测到模组上线,继续检测SIM卡是否插入..."
    	sleep 5
