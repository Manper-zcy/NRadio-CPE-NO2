...
function index()
	entry({"admin", "modem"}, firstchild(), _("内置蜂窝RM520N"), 25).dependent=false
	...
	entry({"admin", "modem", "nets"}, template(template_name), _("状态查询"), 97)
	entry({"admin", "modem", "at"}, template("zmode/at"), _("串口调试"), 98)
	entry({"admin", "modem", "modem"}, cbi("modem"), _("模组设置"), 99) 
